...
STOP_KEEP_NEG = FULL_STOPWORDS - NEGATION_WORDS
STOP_TEXTBOOK = FULL_STOPWORDS

lemmatizer = WordNetLemmatizer()
lemma_cache = {}   # the same words repeat across 30k reviews, so don't lemmatise twice
...
def lemmatize(word, pos):
    ...
    if key not in lemma_cache:
        lemma_cache[key] = lemmatizer.lemmatize(word, pos)
...
def clean_review(text, keep_negation=True):
    ...
    stop = STOP_KEEP_NEG if keep_negation else STOP_TEXTBOOK
    # Tag before filtering, so the tagger still has the stop words around it for context.
    tags = nltk.pos_tag(tokens)
    cleaned = [
        lemmatize(tok, penn_to_wordnet(tag))
        for tok, tag in tags
